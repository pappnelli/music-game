...
export default function RoundWinner() {
  ...
  return (
    <div
      className="relative mb-[5px] flex w-full flex-col items-center gap-1.5 overflow-hidden rounded-2xl border-2 px-4 py-3 text-center shadow-[0_5px_0_0_rgba(20,10,43,0.35)] [animation:pop-in_0.45s_cubic-bezier(0.34,1.56,0.64,1)]"
      style={{
    ...
    >
      <span
        aria-hidden
        className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_30%_15%,rgba(255,255,255,0.5),transparent_55%)]"
      />
      <Sparkles aria-hidden className="absolute top-2 left-3 size-3.5 text-white/80 [animation:pop-in_1.8s_ease-in-out_infinite]" />
      <Sparkles
        aria-hidden
        className="absolute right-3 bottom-2 size-3 text-white/80 [animation:pop-in_2.2s_ease-in-out_infinite] [animation-delay:0.3s]"
      />
      ...
      <span
        className={cn("relative text-xl text-white drop-shadow-lg sm:text-2xl", TEAM_NAME_CLASS)}
        style={{ ...teamNameGlowStyle(winner.color), color: "white" }}
      >
        {winner.name}
      </span>
    </div>
